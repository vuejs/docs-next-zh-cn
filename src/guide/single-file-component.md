...
Vue 单文件组件（又名 `*.vue` 文件，缩写为 **SFC**）是一种特殊的文件格式，它允许将 Vue 组件的模板、逻辑 **与** 样式封装在单个文件中。这是一个 SFC 示例：
...
正如所见，Vue SFC 是经典的 HTML、CSS 与 JavaScript 三重奏的自然扩展。每个 `*.vue` 文件由三种类型的顶级代码块组成：`<template>`、`<script>` 与 `<style>`：

- `<script>` 部分是一个标准的 JavaScript 模块。它应该导出一个 Vue 组件定义作为其默认导出。
- `<template>` 部分定义了组件的模板。
- `<style>` 部分定义了与组件关联的 CSS。

参见 [SFC 语法规范](/api/sfc-spec) 查看更多细节。
...
可以在 [Vue SFC Playground](https://sfc.vuejs.org/) 中使用 SFC 并探索它们是如何编译的。

在实际项目中，通常会将 SFC 编译器与 [Vite](https://vitejs.dev/) 或 [Vue CLI](http://cli.vuejs.org/)（基于 [webpack](https://webpack.js.org/)）等构建工具集成在一起，Vue 提供官方脚手架工具，让你尽快开始使用 SFC。参见 [SFC 工具](/api/sfc-tooling) 部分查看更多细节。

## 为什么要使用 SFC

虽然 SFC 需要一个构建步骤，但也有诸多好处：
