...
- `created`：在绑定元素的 attribute 或事件监听器被应用之前调用。这在指令需要附带事件监听器的情况下很有用——这个事件监听器需要在普通的 `v-on` 事件监听器之前调用。
...
例如你想要创建一个自定义指令，用来通过固定布局将元素固定在页面上。我们可以创建一个自定义指令，它的值以像素为单位更新垂直位置，如下所示：
